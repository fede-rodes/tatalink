...
Meteor.methods({'vote': function(playerGoingOut, playerGoingIn) {
   ...
   // TODO: check if players have the correct status, ie, if playerGoingOut.status === 'playing', ...
   if (!stats) {
   ...
   Players.update({_id: {$in: [playerGoingOut, playerGoingIn]}}, {$inc: {votes: 1}}, {multi: true});
